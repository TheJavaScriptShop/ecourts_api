...
RUN apt-get update; \
    apt-get -y upgrade; \
    apt-get install -y libpq-dev postgresql-server-dev-all gcc python3.9-dev musl-dev python3-psycopg2 build-essential libssl-dev libffi-dev libc-dev openssl cargo xmlsec1 libxmlsec1-dev
RUN apt-get update; \
    apt-get -y upgrade; \
