...
def open_page(driver):
    ...
    while url_trial < 11:
        try:
            ...
            if url_trial >= 10:
                raise Exception(
                    "highcourt: Max url retries exceeded") from exc
        ...
        while is_failed_with_captach:
            try:
                ...
                if total_time.seconds > 300:
                    is_failed_with_captach = False
                    raise Exception(
                        "Message: Highcourt-Maximum time reached") from e_exception
                if counter_retry > 10:
                    is_failed_with_captach = False
                    raise Exception(
                        "Message: Highcourt-Maximum retries reached") from e_exception
        get_cases_trail = 1
        while get_cases_trail <= 11:
            try:
                ...
                if get_cases_trail > 10:
                    raise Exception(
                        "Message: Highcourt-max retries to get case details exceeded") from e_exc
    except Exception as e_exc:
        raise Exception("message: get_highcourt_no_of_cases failed" + "\n" + "start_time: " +
                        start_time.isoformat() + "\n" + "req_body: " + json.dumps(body)) from e_exc


def get_case_details(cases, logger, driver, advoc_name, start_time, body, __location__, case_details, total_downloaded_files, case_sl_no, success_cases):
    failed_cases = []
            ...
            try:
                ...
                if case_detail_trail >= 5:
                    ...
                    try:
                        ...
                        driver.execute_script(
                            "arguments[0].click();", scrollup_element)
                        time.sleep(int(os.environ.get('MIN_WAIT_TIME')))
                    except:
                        pass
...
def get_highcourt_cases_by_name(props):
    try:
        ...
        case_details_data = get_case_details(cases, logger, driver, advoc_name, start_time, body,
        ...
        raise Exception("message: get_highcourt_cases_by_name failed" + "\n" + "start_time: " +
                        start_time.isoformat() + "\n" + "req_body: " + json.dumps(body)) from e_exc
